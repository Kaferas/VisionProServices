...
<form action="{{ route('inventory.update',['inventory' => $inventory]) }}" id="add_form_supply" method="PUT">
    @csrf

    <div class="intro-y flex flex-col sm:flex-row items-center mt-8">
        <input type="text" name="inventory_title"
        class="input input--lg w-full lg:w-64 box pr-10 placeholder-theme-13 "
        placeholder="titre de la sortie" value="{{ $inventory->inventory_title }}">
        ...
        <div class="col-span-12 lg:col-span-6 md:col-span-7">
            <div class="intro-y box">
                ...
                <div class="flex justify-end p-5">
                    ...
                    <button type="button" onclick="handleAddData(this)" class="button bg-theme-1 self-center text-white mt-5 ">Enregistrer</button>
                </div>
...
@section('js_content')
    <script>
        ...
        var inventoryDetails = <?= json_encode($inventoryDetails) ?>;
            ...
            inventoryDetails.forEach(row => {
                html += `
                    <tr id="tr${row.ref_product_code}">
                        <td>
                            ${row.item_inventory_name}
                            <input type="hidden" name="product[]" value="${row.item_inventory_name}">
                            <input type="hidden" name="codebar[]" value="${row.ref_product_code}">
                            <input type="hidden" name="price[]" value="${row.item_inventory_price}">
                        </td>
                        <td>
                            ${row.item_system_quantity ?? 0}
                            <input type="hidden" name="s_quantity[]" value="${row.item_system_quantity ?? 0}" class="input border input--sm w-full">
                        </td>
                        <td>
                            <input type="number" name="p_quantity[]" value="${row.item_physic_quantity ?? 0}" class="input border input--sm w-full">
                        </td>
        ...
        function selectProduct(id) {
            ...
            var product = products.filter(row => row.item_codebar == id)[0];
            ...
            $('#myTbody').append(`
                <tr id="tr${product.item_codebar}">
                    ...
                    <td>
                        ...
                        <input type="hidden" name="s_quantity[]" value="${product.item_quantity ?? 0}" class="input border input--sm w-full">
                    </td>
                    <td>
                        <input type="number" name="p_quantity[]" class="input border input--sm w-full">
                    </td>
                    <td class="table-report__action w-10">
                        <div class="flex justify-center items-center">
